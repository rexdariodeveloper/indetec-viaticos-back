			...
			BEGIN 
				...
				DECLARE @autorizar INT = (
					...
					FROM tblEmpleado AS empleado
							INNER JOIN tblUsuario AS usuario ON empleado.EmpleadoId = usuario.EmpleadoId
							INNER JOIN tblOrganigrama ON AreaAdscripcionId = NodoId
					WHERE usuario.UsuarioId = @creadoPorId
							AND NodoPadreId IS NULL
				)

				IF (@autorizar IS NOT NULL AND @alertaDefinicionId = @ALERTA_DEFINICION_SOLICITUD_VIATICOS_AUTORIZACION_ID)
				BEGIN
					--Si se autorizará generamos la Alerta
