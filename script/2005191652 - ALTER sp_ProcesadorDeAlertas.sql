...
BEGIN
	...
	BEGIN TRY
		--BEGIN TRANSACTION
			...
			DECLARE @referenciaProcesoTempId INT
			...
			BEGIN 
				...
				DECLARE @empleadoAutorizar INT

				IF (@alertaDefinicionId = @ALERTA_DEFINICION_SOLICITUD_VIATICOS_AUTORIZACION_ID)
				BEGIN				
					SELECT @empleadoAutorizar = EmpleadoId
					FROM tblSolicitudViatico
						 INNER JOIN tblOrganigrama ON AreaAdscripcionId = NodoId AND NodoPadreId IS NULL
					WHERE SolicitudViaticoId = @referenciaProcesoId
				END

				IF (@empleadoAutorizar IS NOT NULL)
				BEGIN
					--Insertar la Alerta de Autorización
					...
					VALUES(@alertaDefinicionId, @referenciaProcesoId, @codigoTramite, @textoRepresentativo, @ESTATUS_ALERTA_FINALIZADA_ID, @creadoPorId, GETDATE(), GETDATE())

					--Insertar la Alerta de Solicitud Autorizada
					SET @alertaDefinicionId = @ALERTA_DEFINICION_SOLICITUD_VIATICOS_AUTORIZADA_ID
					SET @textoRepresentativo = 'Solicitud: ' + @codigoTramite + ', Estatus: Autorizado'
					INSERT INTO tblAlerta (AlertaDefinicionId, ReferenciaProcesoId, CodigoTramite, TextoRepresentativo, EstatusId, CreadoPorId, FechaCreacion, FechaFinalizacion)
					VALUES(@alertaDefinicionId, @referenciaProcesoId, @codigoTramite, @textoRepresentativo, @ESTATUS_ALERTA_FINALIZADA_ID, @empleadoAutorizar, GETDATE(), GETDATE())
					...
					BEGIN
						SET @referenciaProcesoTempId = @referenciaProcesoId
						SET @updateQuery = ' UPDATE ' + @nombreTabla + 
											' SET ' + @nombreCampoEstatus + ' = ' + CAST(@valorEstatus AS NVARCHAR(100)) + 
											' WHERE ' + @nombreCampoId + ' = ' + CAST(@referenciaProcesoTempId AS NVARCHAR(100))
						EXEC(@updateQuery)
 					END
				END
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							--Buscamos el id de la solicitud a la que se le cambiara el estatus
							--Si es para enviar a autorizar, autorizar, rechazar o revision una solicitud
							...
							BEGIN
								SET @referenciaProcesoTempId = @referenciaProcesoId
							END
							--Si la definicion alerta es para Asignar viaticos o Enviar a Revision desde Asignacion de Viaticos
							ELSE IF(@alertaDefinicionId = @ALERTA_DEFINICION_ASIGNACION_VIATICOS_ID  )
							BEGIN						
								SET @referenciaProcesoTempId = (SELECT SolicitudViaticoId 
																FROM dbo.tblSolicitudViaticoAsignacion 
						...
						--Obtenemos los correos de los empleados a los que se les notificara por email
			...
			BEGIN
				...
				SET @alertaDefinicionId = (SELECT CASE WHEN @accionId = @ACCION_AUTORIZAR_ALERTA_ID THEN tblAlertaDefinicion.EtapaAccionAlAutorizarId
													   WHEN @accionId = @ACCION_REVISION_ALERTA_ID  THEN tblAlertaDefinicion.EtapaAccionAlRevisionId
													   WHEN @accionId = @ACCION_RECHAZAR_ALERTA_ID  THEN tblAlertaDefinicion.EtapaAccionAlRechazarId END
										   FROM tblAlerta
				...
				BEGIN
					...
					IF(@alertaDefinicionId = @ALERTA_DEFINICION_SOLICITUD_VIATICOS_AUTORIZACION_ID OR 
					   @alertaDefinicionId = @ALERTA_DEFINICION_SOLICITUD_VIATICOS_AUTORIZADA_ID OR
					   @alertaDefinicionId = @ALERTA_DEFINICION_SOLICITUD_VIATICOS_RECHAZADA_ID OR
					   @alertaDefinicionId = @ALERTA_DEFINICION_SOLICITUD_VIATICOS_REVISION_ID)
					BEGIN
						SET @referenciaProcesoTempId = @referenciaProcesoId
					END
					--Si la definicion alerta es para Asignar viaticos o Enviar a Revision desde Asignacion de Viaticos
					ELSE IF(@alertaDefinicionId = @ALERTA_DEFINICION_ASIGNACION_VIATICOS_ID OR 
							@alertaDefinicionId = @ALERTA_DEFINICION_ASIGNACION_VIATICOS_REVISION_ID )
					BEGIN						
						SET @referenciaProcesoTempId = (SELECT SolicitudViaticoId 
													    FROM dbo.tblSolicitudViaticoAsignacion 
